...
const authenticateUser = require("./src/backend/authenticationMiddleware");
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Application is connected to MySQL database" });
  } catch (error) {
...
app.get("/api/election-status", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT election_status FROM election_status WHERE id = 1"
    );
...
app.post("/api/start-election", async (req, res) => {
  try {
    ...
    await pool.query(
      "UPDATE election_status SET election_status = 1 WHERE id = 1"
    );

    res
      .status(200)
      .json({ message: "Election started successfully", status: "started" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal Server Error" });
  }
...
app.post("/api/end-election", async (req, res) => {
  try {
    ...
    await pool.query(
      "UPDATE election_status SET election_status = 2 WHERE id = 1"
    );

    res
      .status(200)
      .json({ message: "Election ended successfully", status: "ended" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal Server Error" });
  }
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (uvcRows.length === 1) {
      ...
      return res
        .status(400)
        .json({ message: "Invalid UVC code or already used", uvcError: true });
    }
  } catch (error) {
    console.error(error);
  ...
  try {
    ...
    if (rows.length === 1) {
      ...
      if (match) {
        ...
        res
          .status(200)
          .json({ message: "Login successful", user: rows[0], token });
      } else {
...
app.get("/api/chosen-details", authenticateUser, async (req, res) => {
  try {
    if (req.user && req.user.voter_id) {
      ...
      res
        .status(401)
        .json({ message: "User not authenticated or missing voter_id" });
    }
  } catch (error) {
      ...
      if (candidateRows.length === 1) {
        ...
        if (req.user && req.user.voter_id) {
          ...
          if (updatedVoterRows.length === 1) {
            ...
            console.error(
              "Error fetching updated voter details after vote submission"
            );
            return res.status(500).json({ message: "Internal Server Error" });
          }
        } else {
        ...
        // Candidate not found
        console.error(
          "Candidate details not found for candidateId:",
          candidateId
        );
...
app.get("/gevs/results", async (req, res) => {
  try {
    ...
    const electionStatusResult = await pool.query(
      "SELECT election_status FROM election_status WHERE id = 1"
    );
    ...
    const parties = Array.from(
      new Set(
        results.flatMap((result) =>
          result.candidates.map((candidate) => candidate.party)
        )
      )
    );
    ...
    seats = parties.map((party) => ({
      ...
      seat: results.reduce((total, result) => {
        const partyResult = result.candidates.find(
    ...
    if (electionStatus === 2) {
      ...
      const totalSeatsWon = seats.reduce(
        (total, party) => total + party.seat,
        0
      );
      ...
      const partiesWithoutMajority = seats.filter(
        (party) => party.seat < halfTotalSeats
      );
      ...
      if (partiesWithoutMajority.length === seats.length) {
        ...
        winner = seats.reduce(
          (max, party) => (party.seat > max.seat ? party : max),
          seats[0]
        ).party;
